...
3.  **처리 (Process):** 메시지의 종류에 따라 미리 정해진 처리 루틴(핸들러)을 실행합니다.
    *   **예시 1:** 사용자가 키보드를 누르면, 하드웨어는 **인터럽트(Interrupt)**라는 긴급 메시지를 보냅니다. 커널은 이 메시지를 받아 '키보드 입력 처리 루틴'을 실행하고, 현재 활성화된 프로그램(메모장 등)에 해당 글자를 전달합니다.
    *   **예시 2:** 프로그램이 파일을 저장하고 싶으면, 커널에게 **시스템 콜(System Call)**이라는 공식적인 요청 메시지를 보냅니다. 커널은 이 메시지를 받아 '파일 시스템 처리 루틴'을 실행하여 데이터를 디스크에 기록합니다.
